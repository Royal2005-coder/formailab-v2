...
export async function generateV3SurveyCreatePayloadFromPrompt(params: {
  ...
  const generation = await generateOrganizationAIObject({
    ...
    temperature: 0.2,
    maxOutputTokens: V3_SURVEY_GENERATE_MAX_OUTPUT_TOKENS,
    timeout: V3_SURVEY_GENERATION_TIMEOUT_MS,
  });

  const generatedSurvey = ZGeneratedSurveyDraft.safeParse(sanitizeGeneratedSurveyDraft(generation.object));
