...
describe("Response Utils", () => {
  ...
  describe("getResponsesJson", () => {
    ...
    test("should handle null response data, null variables, and calculated variables safely without throwing", () => {
      const surveyWithVars: Partial<TSurvey> = {
        ...mockSurvey,
        variables: [{ id: "var1", name: "SCORE" }],
      };

      const responsesWithNulls: Partial<TResponse>[] = [
        {
          id: "response_null",
          surveyId: "survey1",
          data: { q1: null as any },
      ...
      const result = getResponsesJson(
        surveyWithVars as TSurvey,
        responsesWithNulls as TResponse[],
        [["1. Question 1"]],
        [],
        [],
        false
      ...
      expect(result[0]["1. Question 1"]).toBe("");
      expect(result[0]["SCORE"]).toBe("");
      expect(result[1]["1. Question 1"]).toBe("Option 1");
