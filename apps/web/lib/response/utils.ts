...
export const getResponsesJson = (
  ...
  responses.forEach((response, idx) => {
    ...
    elementsHeadlines.forEach((elementHeadline, elementIndex) => {
      const element = elements[elementIndex];
      if (!element) return;
      const answer = response.data ? response.data[element.id] : undefined;
      ...
      if (element.type === "matrix") {
        ...
        elementHeadline.forEach((headline, index) => {
          if (answer && typeof answer === "object" && !Array.isArray(answer)) {
            ...
            const rowLabel = row
              ? getTextContent(getLocalizedValue(row.label, response.language || "default")) ||
                (row.label as Record<string, string>)?.default ||
                (typeof row.label === "object" ? Object.values(row.label || {})[0] : String(row.label)) ||
                ""
              : "";
            const val = rowLabel ? (answer as Record<string, string>)[rowLabel] : undefined;
            jsonData[idx][headline] = val !== undefined ? processResponseData(val as TResponseDataValue) : "";
          } else {
        ...
        jsonData[idx][elementHeadline[0]] = processResponseData(answer as TResponseDataValue);
    ...
    survey.variables?.forEach((variable) => {
      const vars = response.variables as Record<string, unknown> | undefined | null;
      const answer = vars
        ? (vars[variable.id] ?? vars[variable.name] ?? vars[variable.name.toLowerCase()])
        : undefined;
      jsonData[idx][variable.name] = processResponseData(answer as TResponseDataValue);
    });
    ...
    hiddenFields.forEach((field) => {
      const value = response.data ? response.data[field] : undefined;
      if (Array.isArray(value)) {
        jsonData[idx][field] = value.join("; ");
      } else {
        jsonData[idx][field] = processResponseData(value as TResponseDataValue);
      }
    });

    if (survey.isVerifyEmailEnabled) {
      const verifiedEmail = response.data ? response.data["verifiedEmail"] : undefined;
      jsonData[idx]["Verified Email"] = processResponseData(verifiedEmail as TResponseDataValue);
    }
  });
