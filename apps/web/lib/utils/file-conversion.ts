...
export const convertToCsv = async (fields: string[], jsonData: Record<string, string | number>[]) => {
  ...
  // Excel on Windows otherwise guesses an ANSI code page and corrupts Vietnamese.
  // The sep hint also prevents locale-specific Excel installs from placing the
  // whole comma-delimited row into a single cell.
  return `\uFEFFsep=,\r\n${csv.replaceAll("\n", "\r\n")}`;
};
