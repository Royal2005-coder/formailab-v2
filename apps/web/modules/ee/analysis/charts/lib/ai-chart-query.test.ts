...
describe("generateAIChartQuery", () => {
  ...
  test("accepts valueless filters with superfluous AI-generated values", async () => {
    mocks.generateOrganizationAIObject.mockResolvedValueOnce({
      object: {
        name: "AI Feedback Trend Over Time",
        measures: ["FeedbackRecords.count"],
        dimensions: [],
        timeDimensions: [
          {
        ...
        chartType: "line",
        filters: [{ member: "FeedbackRecords.sentiment", operator: "set", values: ["true"] }],
      },
    });

    const result = await generateAIChartQuery({
      organizationId: "organization-1",
      prompt: "AI feedback trend over time",
    });

    expect(result.query.filters).toEqual([{ member: "FeedbackRecords.sentiment", operator: "set" }]);
  });

  test("parses valueless filters that include values so normalization can discard them", () => {
    const result = ZAIQueryResponse.safeParse({
      name: null,
      measures: ["FeedbackRecords.count"],
      dimensions: null,
