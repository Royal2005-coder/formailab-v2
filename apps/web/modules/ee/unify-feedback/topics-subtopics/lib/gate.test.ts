...
describe("computeGate", () => {
  test("insufficient when total open-text records < 500", () => {
    const result = computeGate({ ...ready, fields: [field(450, 450)] });
    expect(result.gateVariant).toBe("insufficient");
    expect(result.totalOpenTextRecords).toBe(450);
  });

  test("sufficient when total open-text records >= 500", () => {
    const result = computeGate({ ...ready, fields: [field(565, 565)] });
    expect(result.gateVariant).toBeNull();
    expect(result.totalOpenTextRecords).toBe(565);
  });
