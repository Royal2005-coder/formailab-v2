...
const getConfiguredHostname = (value) => {
  if (!value) {
    return undefined;
  }
...
const DEVELOPMENT_HOSTS = getUniqueValues([
  ...LOOPBACK_HOSTS,
  getConfiguredHostname(process.env.WEBAPP_URL),
  getConfiguredHostname(process.env.BETTER_AUTH_URL),
]);
...
const nextConfig = {
  ...
  allowedDevOrigins: process.env.NODE_ENV === "production" ? undefined : DEVELOPMENT_HOSTS,
  ...
  async headers() {
    ...
    return [
      ...
      // XLSX templates must keep their native MIME type so Excel/browser clients
      // do not treat the workbook as a CSV download.
      {
        source: "/sample-csv/:file.xlsx",
        headers: [
...
// set actions allowed origins & body size limit
nextConfig.experimental.serverActions = {
  ...(nextConfig.experimental.serverActions ?? {}),
  allowedOrigins: process.env.WEBAPP_URL ? [process.env.WEBAPP_URL.replace(/https?:\/\//, "")] : undefined,
  bodySizeLimit: "50mb",
};
nextConfig.serverActions = {
  bodySizeLimit: "50mb",
};
