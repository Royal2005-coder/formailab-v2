...
Để ChatGPT hoặc AI Agent tạo được file nhập chuẩn, hãy thực hiện:

### Bước 1: Tải file Skill.md này
- Mở file `AI_LAB_SURVEY_IMPORT_SKILL.md` (file này).
- Tải lên như tệp đính kèm trong cuộc trò chuyện.

### Bước 2: Tải file Excel template
- Tải file `AILAB_Full_Adaptive_Survey_Template.xlsx` và đính kèm cùng.
- Hoặc file CSV mẫu: `formbricks-survey-import-template.csv`.

### Bước 3: Yêu cầu ChatGPT tạo test bank
```
Tôi đã đính kèm Skill.md (hướng dẫn chuẩn) và file Excel template.
Hãy tạo test bank adaptive response với:
- [Số] câu hỏi trắc nghiệm, [Số] câu hỏi tự luận
- Biểu thức rẽ nhánh LimeScript AST
- Ma trận điểm SPSS
- Ngôn ngữ: tiếng Việt
...
- Tải file kết quả.
- Nạp tại **AI Lab → Survey Import** (tối đa 10 MiB).
- Hệ thống preview trước khi commit.
...
Trước khi bàn giao file đã tạo, kiểm tra:
...
9. Ngôn ngữ mặc định: `vi` (tiếng Việt).
