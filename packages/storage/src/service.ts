...
import { S3_BUCKET_NAME, S3_ENDPOINT_URL } from "./constants";
...
> => {
  try {
    ...
    let browserSignedUrl = url;
    if (publicUrl) {
      const publicBase = publicUrl.replace(/\/$/, "");
      const internalBase = S3_ENDPOINT_URL
        ? S3_ENDPOINT_URL.replace(/\/$/, "")
        : "http://surveyops-minio:9000";
      browserSignedUrl = url.startsWith(internalBase)
        ? url.replace(internalBase, publicBase)
        : url.replace("http://surveyops-minio:9000", publicBase);
    } else if (url.includes("surveyops-minio:9000")) {
      browserSignedUrl = url.replace("http://surveyops-minio:9000", "http://127.0.0.1:9100");
    }

    return ok({
      signedUrl: browserSignedUrl,
