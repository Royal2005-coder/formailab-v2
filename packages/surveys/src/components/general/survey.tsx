...
export function Survey({
  ...
  const [blockId, setBlockId] = useState(() => {
    if (startAtQuestionId) {
      ...
      return (
        startBlock?.id ||
        localSurvey.blocks.find((b) => b.elements.length > 0)?.id ||
        localSurvey.blocks[0]?.id
      );
    } else if (localSurvey.welcomeCard.enabled) {
      return "start";
    }

    const firstDisplayableBlock = localSurvey.blocks.find((b) => b.elements.length > 0);
    return firstDisplayableBlock?.id || localSurvey.blocks[0]?.id;
  });
  ...
  const evaluateLogicAndGetNextBlockId = (
    ...
    // LimeSurvey evaluates calculated variables before relevance/jump rules on the
    // same page.  Do the same here so a result block can route using the value it
    // calculates in its first (unconditional) rule during this navigation pass.
    for (const logic of currentBlock.logic ?? []) {
      if (logic.conditions.conditions.length !== 0) continue;
      const computeActions = logic.actions.filter(
        (action) => action.objective === "evaluateExpression" || action.objective === "calculate"
      );
      if (computeActions.length === 0) continue;
      const result = performActions(localSurvey, computeActions, localResponseData, calculationResults);
      calculationResults = { ...calculationResults, ...result.calculations };
    }
    ...
    // Helper to check if any condition leftOperand element ID exists in responseData
    const hasAnsweredConditionInputs = (
      logicRules: TSurveyBlockLogic[] | undefined,
      data: TResponseData
    ): boolean => {
      if (!logicRules || logicRules.length === 0) return false;
      for (const rule of logicRules) {
        const condGroup = rule.conditions;
        if (!condGroup || !condGroup.conditions) continue;
        for (const cond of condGroup.conditions) {
          if (cond && "leftOperand" in cond && cond.leftOperand && cond.leftOperand.type === "element") {
            const elemId = cond.leftOperand.value;
            if (data[elemId] !== undefined && data[elemId] !== null && data[elemId] !== "") {
              return true;
            }
          }
        }
    ...
    const evaluateBlockLogic = () => {
      ...
      // Use logicFallback if no jump target was set and fallback target is a valid block or ending.
      if (
        !firstJumpTarget &&
        currentBlock.logicFallback &&
        (currentBlock.elements.length > 0 ||
          hasAnsweredConditionInputs(currentBlock.logic, localResponseData)) &&
        (localSurvey.blocks.some((b) => b.id === currentBlock.logicFallback) ||
          localSurvey.endings.some((e) => e.id === currentBlock.logicFallback))
      ) {
    ...
    // Return the jump target (which is a block ID) or the next block in sequence with elements
    let nextBlockId = firstJumpTarget;
    if (!nextBlockId) {
      const nextContentBlock = localSurvey.blocks.find(
        (b, idx) => idx > currentBlockIndex && b.elements.length > 0
      );
      nextBlockId = nextContentBlock?.id || localSurvey.blocks[currentBlockIndex + 1]?.id || firstEndingId;
    }
  ...
  // Auto-advance past empty synthetic routing blocks (blocks with no UI elements)
  useEffect(() => {
    if (blockId === "start" || blockId === "end") return;
    const isEndingCard = localSurvey.endings.some((e) => e.id === blockId);
    if (isEndingCard) return;

    const block = localSurvey.blocks.find((b) => b.id === blockId);
    if (block && block.elements.length === 0) {
      const { nextBlockId, calculatedVariables } = evaluateLogicAndGetNextBlockId({});
      if (Object.keys(calculatedVariables).length > 0) {
        setCurrentVariables((prev) => ({ ...prev, ...calculatedVariables }));
      }
      if (nextBlockId && nextBlockId !== blockId) {
        ...
        if (nextContentBlock) {
          setBlockId(nextContentBlock.id);
        }
      }
    }
  }, [blockId, localSurvey, currentBlockIndex]);

  // Entry-time evaluation: when entering a block with elements, run evaluateExpression/calculate
  // actions from logic rules whose conditions are met. This ensures computed variables (e.g. VCSCORE)
  // are evaluated before the block renders, not just on submit. Only runs on blockId change.

  useEffect(() => {
    if (blockId === "start" || blockId === "end") return;
    const block = localSurvey.blocks.find((b) => b.id === blockId);
    if (!block || block.elements.length === 0) return;

    const localResponseData = { ...responseData };
    let calculations = { ...currentVariables };
    let changed = false;

    for (const logic of block.logic ?? []) {
      const isMet = evaluateLogic(
        localSurvey,
        localResponseData,
        calculations,
        logic.conditions,
      ...
      const computeActions = logic.actions.filter(
        (a) => a.objective === "evaluateExpression" || a.objective === "calculate"
      );
      if (computeActions.length === 0) continue;

      const { calculations: ruleCalcs } = performActions(
        localSurvey,
        computeActions,
        localResponseData,
        calculations
      );
      if (Object.keys(ruleCalcs).length > 0) {
        calculations = { ...calculations, ...ruleCalcs };
        changed = true;
      }
    }
    ...
    if (changed) {
      setCurrentVariables((prev) => {
        const hasChanges = Object.entries(calculations).some(([key, value]) => prev[key] !== value);
        return hasChanges ? { ...prev, ...calculations } : prev;
      });
    }
  }, [blockId, localSurvey, responseData, currentVariables, selectedLanguage]);
  ...
  const getCardContent = (blockIdx: number, offset: number): JSX.Element | undefined => {
    ...
    const content = () => {
      if (blockIdx === -1) {
        ...
        const endingCard =
          localSurvey.endings.find((ending) => ending.id === blockId) ?? localSurvey.endings[0];
        if (endingCard) {
          return (
            <EndingCard
              survey={localSurvey}
