  ...
  actions.forEach((action) => {
    switch (action.objective) {
      ...
      case "evaluateExpression":
        calculations[action.variableId] = evaluateExpressionAction(action, data, calculations);
        break;
      case "requireAnswer":
...
const evaluateSingleCondition = (
  ...
  try {
    ...
    const isLeftNumeric =
      (condition.leftOperand.type === "variable" && (leftField as TSurveyVariable)?.type === "number") ||
      (condition.leftOperand.type === "element" &&
        (leftField as TSurveyElement)?.type === TSurveyElementTypeEnum.OpenText &&
        (leftField as any)?.inputType === "number") ||
      typeof leftValue === "number";

    const isRightNumeric =
      (condition.rightOperand?.type === "variable" && (rightField as TSurveyVariable)?.type === "number") ||
      (condition.rightOperand?.type === "element" &&
        (rightField as TSurveyElement)?.type === TSurveyElementTypeEnum.OpenText &&
        (rightField as any)?.inputType === "number") ||
