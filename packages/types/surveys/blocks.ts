...
const ZExpressionReferenceTarget = z.object({
  type: z.enum(["element", "variable"]),
  value: ZId,
});

export const ZActionEvaluateExpression = z.object({
  id: ZId,
  objective: z.literal("evaluateExpression"),
  variableId: ZId,
  ...
  references: z.array(
    z.object({
      source: z
        .string()
        .min(1)
        .regex(/^[A-Za-z][A-Za-z0-9_-]*(?:\.NAOK)?$/),
      target: ZExpressionReferenceTarget,
      missingValue: z.enum(["zero", "error"]),
      valueMap: z.record(z.string(), z.union([z.string(), z.number()])).optional(),
    })
  ),
});

export type TActionEvaluateExpression = z.infer<typeof ZActionEvaluateExpression>;
