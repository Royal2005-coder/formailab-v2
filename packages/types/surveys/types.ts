...
export const ZSurveyLogicAction = z.union([
  ZActionCalculate,
  ZActionEvaluateExpression,
...
const validateActions = (
  ...
  const actionIssues: (z.core.$ZodIssue | undefined)[] = actions.map((action) => {
    ...
    } else if (action.objective === "evaluateExpression") {
      const previousQuestionIds = new Set(previousQuestions.map((question) => question.id));
      const variableIds = new Set(survey.variables.map((variable) => variable.id));
      const invalidReference = action.references.find(({ target }) =>
        target.type === "element" ? !previousQuestionIds.has(target.value) : !variableIds.has(target.value)
      );

      if (invalidReference) {
        return {
          code: "custom",
          message: `Expression reference ${invalidReference.target.value} does not exist before logic no: ${String(logicIndex + 1)} of question ${String(questionIndex + 1)}`,
          path: ["questions", questionIndex, "logic", logicIndex],
        };
      }
    } else {
...
const validateBlockActions = (
  ...
  const actionIssues: (z.core.$ZodIssue | undefined)[] = actions.map((action) => {
    if (action.objective === "calculate") {
      ...
      const variable = survey.variables.find((candidate) => candidate.id === action.variableId);
      if (!variable || variable.type !== "number") {
        return {
          code: "custom",
          message: `Conditional Logic: Expression output variable ${action.variableId} must be an existing number variable in logic no: ${String(logicIndex + 1)} of block ${String(blockIndex + 1)}`,
          path: ["blocks", blockIndex, "logic", logicIndex],
        };
      }
      ...
      const invalidReference = action.references.find(({ target }) =>
        target.type === "element"
          ? !allElements.has(target.value)
          : !survey.variables.some((candidate) => candidate.id === target.value)
      );
      if (invalidReference) {
        return {
          code: "custom",
          message: `Conditional Logic: Expression reference target ${invalidReference.target.value} does not exist in logic no: ${String(logicIndex + 1)} of block ${String(blockIndex + 1)}`,
          path: ["blocks", blockIndex, "logic", logicIndex],
        };
      }
